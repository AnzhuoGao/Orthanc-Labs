#!/usr/bin/env python3
"""
Real-time 3D Voxel Grid Visualizer for Orthanc Labs Drone Detection System (macOS Version)

This script is optimized for macOS and uses native Cocoa/OpenGL instead of GLX.

Usage:
    python visualize_voxel_grid_macos.py [threshold] [zmq_port]
...
# Set macOS OpenGL platform before importing OpenGL
...
# Camera control class (same as before)
class Camera:
    def __init__(self):
...
# Detection tracking for trails (same as before)
...
# Simplified VoxelGrid for demo purposes
class VoxelGrid:
    def __init__(self):
        self.grid = None
    ...
    def create_demo_data(self):
        """Create some demo voxel data for testing"""
        self.grid_size = [50, 50, 25]
        self.voxel_size = 1.0
        self.grid_origin = [-25.0, -25.0, 0.0]
        
        # Create a grid with some demo objects
        grid_volume = self.grid_size[0] * self.grid_size[1] * self.grid_size[2]
        self.grid = np.zeros(self.grid_size, dtype=np.float32)
        
        # Add some demo "drone" objects
        import time
        t = time.time() * 0.5
        
        # Moving object 1
        x1 = int(25 + 15 * np.cos(t))
        y1 = int(25 + 10 * np.sin(t))
        z1 = int(10 + 3 * np.sin(t * 2))
        
        if 0 <= x1 < self.grid_size[0] and 0 <= y1 < self.grid_size[1] and 0 <= z1 < self.grid_size[2]:
            for dx in range(-2, 3):
                for dy in range(-2, 3):
                    for dz in range(-1, 2):
                        nx, ny, nz = x1 + dx, y1 + dy, z1 + dz
                        if 0 <= nx < self.grid_size[0] and 0 <= ny < self.grid_size[1] and 0 <= nz < self.grid_size[2]:
                            self.grid[nx, ny, nz] = 0.8 - abs(dx) * 0.1 - abs(dy) * 0.1 - abs(dz) * 0.1
        
        # Moving object 2
        x2 = int(25 + 12 * np.cos(t * 1.3 + 1))
        y2 = int(25 + 8 * np.sin(t * 1.3 + 1))
        z2 = int(8 + 2 * np.sin(t * 3 + 1))
        
        if 0 <= x2 < self.grid_size[0] and 0 <= y2 < self.grid_size[1] and 0 <= z2 < self.grid_size[2]:
            for dx in range(-1, 2):
                for dy in range(-1, 2):
                    for dz in range(-1, 2):
                        nx, ny, nz = x2 + dx, y2 + dy, z2 + dz
                        if 0 <= nx < self.grid_size[0] and 0 <= ny < self.grid_size[1] and 0 <= nz < self.grid_size[2]:
                            self.grid[nx, ny, nz] = 0.6 - abs(dx) * 0.1 - abs(dy) * 0.1 - abs(dz) * 0.1
        
        # Create demo detections
        self.detections = [
            {
                'id': 1,
                'position': np.array([x1 - 25, y1 - 25, z1]) * self.voxel_size,
                'confidence': 0.85,
                'velocity': np.array([np.cos(t) * 2, np.sin(t) * 2, 0.5])
            },
            {
                'id': 2,
                'position': np.array([x2 - 25, y2 - 25, z2]) * self.voxel_size,
                'confidence': 0.72,
                'velocity': np.array([np.sin(t) * 1.5, np.cos(t) * 1.5, 0.3])
            }
        ]
    ...
    def update_from_zmq(self, voxel_data, detection_data=None):
        """Update voxel grid from ZMQ data (same as before)"""
        with self.mutex:
            try:
                # Parse voxel data
    ...
    def _parse_detections(self, detection_data):
        """Parse detection data from binary format (same as before)"""
        detections = []
        try:
            offset = 0
    ...
    def draw_detections(self, tracker):
        """Draw detected drones with enhanced visualization (same as before)"""
...
# ZeroMQ live listener (with fallback to demo mode)
def zmq_listener(port=5556):
    ...
    try:
        ...
        while running:
            try:
                ...
                if not zmq_connected:
                    # No ZMQ data available, update demo data
                    voxel_grid.create_demo_data()
                time.sleep(0.01)
                continue
            except Exception as e:
        ...
        print(f"ZMQ not available, running in demo mode: {e}")
        # Run in demo mode
        while running:
            voxel_grid.create_demo_data()
            time.sleep(0.1)

# Keyboard and mouse handlers (same as before)
...
# Process keyboard input and move camera (same as before)
def process_input():
    speed = camera.speed
...
# Draw coordinate axes and reference grid (same as before)
def draw_reference():
    # Draw axes
    glLineWidth(3.0)
...
# OpenGL display function (same as before)
def display():
...
def main():
    ...
    glutCreateWindow("Orthanc Labs - 3D Drone Detection Visualizer (macOS)")
    ...
    print("Orthanc Labs - 3D Drone Detection Visualizer (macOS)")
    print("=" * 55)
    print("Controls:")
    ...
    print(f"  Running in demo mode (animated objects)")
    print("=" * 55)
