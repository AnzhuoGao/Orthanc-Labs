#!/usr/bin/env python3
"""
Simple Cross-Platform Voxel Grid Visualizer for Orthanc Labs Bird Detection
...
Now includes velocity and acceleration vector visualization for tracked birds.
...
@dataclass
class BirdState:
    """Represents a bird's state at a specific time"""
...
@dataclass
class BirdVectors:
    """Calculated velocity and acceleration vectors for a bird"""
...
class BirdTracker:
    """Tracks bird positions over time to calculate velocity and acceleration"""
    ...
    def __init__(self, max_history=10, dt=0.1):
        ...
        self.bird_histories = defaultdict(lambda: deque(maxlen=max_history))
        self.next_bird_id = 0
        
    def update_birds(self, bird_positions: List[Tuple[np.ndarray, float]], current_time: float) -> List[BirdVectors]:
        """
        Update bird positions and calculate velocity/acceleration vectors
        
        Args:
            bird_positions: List of (position, intensity) tuples
            current_time: Current timestamp
            
        Returns:
            List of BirdVectors with calculated velocity and acceleration
        """
        # Simple nearest-neighbor tracking (for demo purposes)
        bird_vectors = []
        
        # Update existing birds or create new ones
        used_positions = set()
        
        for bird_id, history in list(self.bird_histories.items()):
            if len(history) == 0:
                continue
            ...
            for i, (pos, intensity) in enumerate(bird_positions):
            ...
            if best_match is not None:
                ...
                new_state = BirdState(best_match[0], current_time, best_match[1])
                history.append(new_state)
                
                # Calculate vectors
                vectors = self._calculate_vectors(history)
                if vectors is not None:
                    bird_vectors.append(vectors)
        
        # Create new birds for unmatched positions
        for i, (pos, intensity) in enumerate(bird_positions):
            if i not in used_positions:
                bird_id = self.next_bird_id
                self.next_bird_id += 1
                
                new_state = BirdState(pos, current_time, intensity)
                self.bird_histories[bird_id].append(new_state)
        
        return bird_vectors
    
    def _calculate_vectors(self, history: deque) -> Optional[BirdVectors]:
...
class SimpleDemoData:
    ...
    def generate_frame(self):
        """Generate a frame of demo data with realistic bird trajectories"""
        ...
        # Store bird positions for tracking
        self.current_bird_positions = []
        
        # Generate birds with realistic trajectories
        for i, params in enumerate(self.bird_params):
            position = self._calculate_bird_position(t, params, i)
            x, y, z = position
            ...
            self.current_bird_positions.append((world_pos, intensity))
    ...
    def _calculate_bird_position(self, t: float, params: dict, bird_id: int) -> np.ndarray:
        """Calculate realistic bird position based on trajectory type"""
        offset = bird_id * 0.5  # Phase offset for each bird
    ...
    def get_current_bird_positions(self) -> List[Tuple[np.ndarray, float]]:
        """Get the current frame's bird positions for tracking"""
        return getattr(self, 'current_bird_positions', [])
    
    def _add_object_optimized(self, grid, cx, cy, cz, intensity=0.8, size=2):
        """Add a bird-like object to the grid (vectorized for performance)"""
...
class BirdVoxelVisualizer:
    """Simple matplotlib-based voxel visualizer with bird tracking"""
    ...
    def __init__(self, threshold=0.3, demo_mode=True):
        ...
        self.bird_tracker = BirdTracker(max_history=8, dt=0.1)
        self.mutex = Lock()
    ...
    def setup_tactical_ui(self):
        ...
        self.ax.set_title("ORTHANC LABS - BIRD DETECTION MONITORING", 
...
└ STATUS: ACTIVE"""
    ...
    def update_plot(self):
        ...
        # Update bird tracking for demo mode (regardless of grid state)
        if self.demo_mode:
            if hasattr(self.demo_data, 'current_bird_positions'):
                current_time = time.time()
                bird_positions = self.demo_data.get_current_bird_positions()
                if len(bird_positions) > 0:  # Only track if we have positions
                    with self.mutex:
                        self.current_vectors = self.bird_tracker.update_birds(bird_positions, current_time)
    ...
    def _apply_tactical_theme(self):
        ...
        # Bird detection interface colors (nature-inspired)
        self.colors = {
            'bg_primary': '#0f1419',      # Dark forest background
            'bg_secondary': '#1e2328',    # Panel backgrounds
            'accent_primary': '#56d364',   # Green accent (nature)
            ...
            'highlight': '#58a6ff',       # Blue highlights
    ...
    def _draw_bird_vectors(self):
        """Draw velocity and acceleration vectors for tracked birds (smooth updates)"""
        ...
        valid_vectors = []
        for bird_vector in vectors:
            pos = bird_vector.position
            vel = bird_vector.velocity
            accel = bird_vector.acceleration
            confidence = bird_vector.confidence
            
            # More lenient thresholds for demo
            if confidence >= 0.2:  # Require some confidence
                valid_vectors.append(bird_vector)
        ...
        if should_update:
            ...
            # Draw new vectors
            for bird_vector in valid_vectors:
                pos = bird_vector.position
                vel = bird_vector.velocity
                accel = bird_vector.acceleration
    ...
    def run(self):
        ...
        print("Starting Orthanc Labs Bird Detection Visualizer...")
        print("=" * 50)
        print("Controls:")
...
def main():
    ...
    visualizer = BirdVoxelVisualizer(threshold=threshold, demo_mode=demo_mode)
    visualizer.run()
